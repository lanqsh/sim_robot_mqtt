...
// 发送Lora参数&清扫设置上报
export async function sendLoraCleanSettingsReport(robotId, serialNumber) {
    if (!robotId && !serialNumber) {
        alert('请填写机器人ID或序号（二选一）');
        return false;
    }
    ...
    const robotInfo = robotId ? `机器人ID: ${robotId}` : `机器人序号: ${serialNumber}`;
    const confirmMsg = `确定发送Lora参数&清扫设置上报吗？\n\n${robotInfo}`;
    ...
    try {
        ui.showLoading('正在发送Lora参数&清扫设置上报...');

        const identifier = robotId || serialNumber;
        const identifierType = robotId ? 'id' : 'serial';
        const result = await api.sendLoraCleanSettingsReport(identifier, identifierType);

        ui.hideLoading();

        if (result.success) {
            alert(`Lora参数&清扫设置上报发送成功！\n\n机器人: ${result.robot_id}\n\n这是一个上报类指令，平台不回复。`);
            return true;
        } else {
            alert('发送失败: ' + result.error);
            return false;
        ...
        ui.hideLoading();
        console.error('发送Lora参数&清扫设置上报失败:', error);
        alert('发送失败: ' + error.message);
        return false;
    }
}
